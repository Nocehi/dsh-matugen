...
The revision hashes the normalized token layer in sorted token-name order. Whitespace or key-order changes in `dms-colors.json` therefore do not repaint the client; a semantic color change does.

## Why an HTTP route

The source palette is a host filesystem artifact while `ThemeRuntime` lives in the browser. The existing DSH Web server already owns the browser origin, so a small exact GET/HEAD route keeps localhost and reverse-proxied/Tailscale Web sessions on the same origin without adding a second daemon, port, CORS policy, or DSH business-RPC method.
